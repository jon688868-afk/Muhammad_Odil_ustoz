...
(function () {
  ...
  function renderHomePage() {
    ...
    var newsHTML = news.slice(0, 6).map(function (n) {
      var thumbStyle = n.image ? '' : 'background:linear-gradient(135deg,' + n.color + ',#061e1e)';
  ...
  function renderPubList(pubs) {
    ...
    return '<div style="display:flex;flex-direction:column;gap:12px">'
      + pubs.map(function (p) {
          ...
          var spineStyle = p.coverImage ? 'padding:0;overflow:hidden' : 'background:linear-gradient(135deg,' + p.color + ',#061e1e)';
  ...
  function renderGalleryPage() {
    ...
    var itemsHTML = items.map(function (g) {
      ...
      var bgStyle = g.coverImage ? '' : 'background:linear-gradient(135deg,' + g.color + ',#061e1e);';
  ...
  function renderArchivePage() {
    ...
    var itemsHTML = allNews.map(function (n) {
      var thumbStyle = n.image ? 'overflow:hidden' : 'background:linear-gradient(135deg,' + n.color + ',#061e1e)';
      var thumbContent = n.image
        ? '<img src="' + escAttr(n.image) + '" alt="" style="width:100%;height:100%;object-fit:cover" />'
        : n.icon;
  ...
  function openPubModal(id) {
    ...
    document.getElementById('modal-content').innerHTML =
      ...
      + '<div style="width:90px;height:120px;background:linear-gradient(135deg,' + p.color + ',#061e1e);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:3px 4px 14px rgba(0,0,0,.3)">'
    ...
    if (saved.length === 0) {
      ...
      bodyHTML = saved.map(function (p) {
        return '<div style="display:flex;gap:12px;padding:12px 0;border-bottom:1px solid var(--border-1);align-items:center">'
          + '<div style="width:40px;height:54px;background:linear-gradient(135deg,' + p.color + ',#061e1e);border-radius:6px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:2px 2px 8px rgba(0,0,0,.2)">'
